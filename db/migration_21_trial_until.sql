-- MIGRATION 21 — Trial até uma data específica (painel Master)
-- app_settings.trial_until (date, opcional): se preenchida, novas empresas
-- recebem trial até o fim deste dia, com prioridade sobre trial_days.
ALTER TABLE public.app_settings ADD COLUMN IF NOT EXISTS trial_until date;
...
begin
  ...
  select trial_days, trial_until into v_trial_days, v_trial_until from public.app_settings where id = 1;
  v_trial_end := coalesce(
    (v_trial_until + time '23:59:59') at time zone 'America/Sao_Paulo',
  ...
  values (v_cid, v_free_plan, 'trial', v_trial_end, v_trial_end, 0);
