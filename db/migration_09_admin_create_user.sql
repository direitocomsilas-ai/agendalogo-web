-- Painel Master: criação manual de usuário (e-mail + senha) pelo administrador
-- create_company ganha p_owner_id para permitir criar a empresa em nome do novo usuário
...
-- Cria usuário (e-mail + senha) manualmente pelo Master; opcionalmente já cria a empresa com dados demo
create or replace function public.admin_create_user(
  p_name text, p_email text, p_password text, p_whatsapp text default '',
  p_company_name text default '', p_category text default 'Outro'
) returns jsonb
...
begin
  if not public.is_master() then raise exception 'Acesso restrito ao administrador'; end if;
  if p_password is null or length(p_password) < 8 then raise exception 'A senha deve ter pelo menos 8 caracteres'; end if;
  select id into v_uid from auth.users where lower(email) = v_email;
  if v_uid is not null then raise exception 'Já existe um usuário com este e-mail'; end if;
  ...
  values ('00000000-0000-0000-0000-000000000000', gen_random_uuid(), 'authenticated', 'authenticated',
    ...
    jsonb_build_object('name', p_name, 'whatsapp', p_whatsapp),
    now(), now())
  returning id into v_uid;

  insert into auth.identities (id, user_id, provider_id, provider, identity_data, last_sign_in_at, created_at, updated_at)
  values (gen_random_uuid()::text, v_uid, 'email', 'email',
    jsonb_build_object('sub', v_uid::text, 'email', v_email, 'email_verified', true),
    now(), now(), now());

  if coalesce(p_company_name, '') <> '' then
    v_cid := public.create_company(p_company_name, p_category, p_whatsapp, '', null, v_uid);
  end if;

  return jsonb_build_object('user_id', v_uid, 'company_id', v_cid, 'email', v_email);
end $$;

-- Lista de e-mails dos usuários (visível apenas no painel Master)
